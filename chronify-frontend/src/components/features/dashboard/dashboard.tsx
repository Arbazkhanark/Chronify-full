...
interface GtagEventParams {
  ...
  user_status: 'new_user' | 'returning_user'
}
...
export default function DashboardClient() {
  ...
  const structuredData = {
    ...
    name: 'User Dashboard',
    description:
      'Personal productivity dashboard for tracking goals and tasks',
    publisher: {
      '@type': 'Organization',
      name: 'Chronify',
      logo: {
  ...
  return (
    <>
      <Head>
        <title>
          {user.name
            ? `${user.name}'s Dashboard - Chronify`
            : 'Dashboard - Chronify'}
        </title>
        ...
        <meta
          ...
          content={
            user.name
              ? `${user.name}'s personal productivity dashboard on Chronify. Track daily goals, manage tasks, and monitor consistency.`
              : 'Personal dashboard to track your daily goals, tasks, and consistency metrics on Chronify.'
          }
        />
                ...
                {user.onboardingStep === 4 && (
                  <div className="mt-4 p-4 rounded-xl bg-gradient-to-r from-green-500/10 to-emerald-500/10 border border-green-500/20">
                    <p className="text-sm text-green-600">
                      🎯{' '}
                      <strong>Pro Tip:</strong> Start by setting
                      your first goal to get personalized
                      recommendations.
                    </p>
                  </div>
                )}
                ...
                <div className="flex-1">
                  ...
                  >
                    {guideItems.map((item) => (
                      <div
                        key={item.step}
                        className="p-4 rounded-xl bg-background/50 border border-border hover:border-primary/30 transition-all"
                      >
