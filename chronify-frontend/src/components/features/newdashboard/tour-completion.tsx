// src/components/features/newdashboard/tour-completion.tsx
'use client'
...
type CelebrationType = 'onboarding' | 'regular'
...
export function TourCompletionCelebration() {
  ...
  const [celebrationType, setCelebrationType] =
    useState<CelebrationType>('regular')
  ...
  useEffect(() => {
    ...
    const checkTourCompletion = async () => {
      try {
        ...
        const isNewUser = currentUser?.onboardingStep === 4

        if (
          (tourCompleted || onboardingTourCompleted) &&
          !celebrationShown
        ) {
        ...
        console.error(
          'Failed to check tour completion:',
          error
        )
  ...
  const getCelebrationContent = (): CelebrationContent => {
    if (celebrationType === 'onboarding') {
      return {
        title: 'Profile Complete! 🎉',
        subtitle:
          "You're now ready to make the most of Chronify AI",
    ...
    return {
      title: 'Tour Complete! ✨',
      subtitle:
        "You're now familiar with all the features",
            ...
            <div className="text-center">
              ...
              {celebrationType === 'onboarding' && (
                <p className="text-xs text-muted-foreground mt-4">
                  ...
                  You'll get personalized tips based on
                  your profile
                </p>
              )}
            </div>
